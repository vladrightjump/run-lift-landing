...
| _neaplicată_ | `runlift_turnstile_lockdown` | runlift | **Nu e aplicată încă** — se rulează ULTIMA din deploy-ul anti-bot (vezi `ANTI-BOT.md`). Scoate politicile și grant-urile de INSERT pentru `anon` de pe `registrations`, `event_waitlist` și `launch_notifications`: după ea, singura cale de scriere publică e funcția Edge `submit-form`, care verifică un token Turnstile înainte de insert. Trigger-ele guard (`registrations_guard_trg`, `event_waitlist_cap_trg`, `*_forteaza_editia`) rămân active — sunt trigger-e, nu politici, deci se aplică și scrierilor cu cheia de service. `/admin` (RPC-uri `SECURITY DEFINER` cu `p_token`), `public_stats`, `public_config`, `confirm_signup` și `unsubscribe` nu sunt atinse. Rollback în DOI pași (întâi frontendul, apoi SQL-ul) la finalul fișierului. Vezi `supabase-migration-turnstile-lockdown.sql` |

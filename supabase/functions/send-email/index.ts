...
//  • "anunt"     — anunțul unei ediții noi către toți participanții de până acum,
//                  o dată pe persoană, cu dezabonare; token de admin. Lista o
//                  rezolvă `runlift.anunt_recipients()`, nu clientul.
//
...
/* ---------- Anunțul către toți participanții de până acum ---------- */
...
async function trimiteCuReincercare(
  ...
  const r = await sendOne(m, badge, unsubPage, unsubApi);
  if (r.status !== 429) return r;
  await pauza(1000);
  return await sendOne(m, badge, unsubPage, unsubApi);
}

const linkDezabonarePagina = (token?: string): string | undefined =>
  token ? `https://parktraining.fit/unsubscribe?token=${token}` : undefined;

const linkDezabonareApi = (token?: string): string | undefined =>
  token ? `${SUPABASE_URL}/functions/v1/unsubscribe?token=${token}` : undefined;
...
Deno.serve(async (req: Request) => {
  ...
  if (mode === "replay") {
    ...
    // `admin_replay_lookup` îl refuză cu `mod_exclus`; aici îl preia căutarea
    // dedicată, care întoarce subiectul și textul din jurnal plus tokenul de
    // dezabonare de ACUM — dacă între timp s-a dezabonat, refuzul e al ei.
    if (!plan.ok && plan.mod === "anunt") {
      const gasit = await rpc<
        {
          ok: boolean;
          motiv: string | null;
          email: string;
      ...
      const anunt = gasit && gasit[0];
      if (!anunt) return json(500, { error: "lookup_failed" });
      if (!anunt.ok) return json(409, { error: "not_replayable", motiv: anunt.motiv });

      const badge = await loadBadge();
      const r = await trimiteCuReincercare(
        { to: anunt.email, subject: anunt.subiect, text: anunt.text_email },
        badge,
        linkDezabonarePagina(anunt.token_unsub),
        linkDezabonareApi(anunt.token_unsub)
      );
      await logSends([
        {
          email: anunt.email,
          nume: anunt.nume,
          subiect: anunt.subiect,
          text_email: anunt.text_email,
          mod: "anunt",
          audienta: "istoric",
          ok: r.ok,
          provider_status: r.status,
      ...
      return json(200, { sent: r.ok ? 1 : 0, failed: r.ok ? 0 : 1, mod: "anunt" });
    }
  ...
  // ---- ANUNT: ediție nouă, către toți participanții de până acum ----
  //
  ...
  if (mode === "anunt") {
    ...
    if (
      ...
      return json(400, { error: "bad_exclude" });
    }

    const destinatari = await rpc<DestinatarAnunt[]>("anunt_recipients", { p_exclude: exclude });
    // `null` = RPC-ul a picat. O listă goală ar fi o afirmație („nimeni de
    // anunțat"), iar aici nu știm nimic.
    if (destinatari === null) return json(500, { error: "recipients_failed" });
    ...
    if (!subiectBrut || !textBrut) return json(400, { error: "missing_content" });
    // Listă închisă: cheia ajunge în jurnal, de unde o citește rejucarea.
    const sablon = payload.sablon === "bulk_participant_anunt" ? "bulk_participant_anunt" : undefined;
    ...
    if (testTo) {
      if (!EMAIL_RE.test(testTo)) return json(400, { error: "bad_test_to" });
      ...
      const body = fillVars(text, model?.nume ?? "Test", testTo);
      const r = await trimiteCuReincercare(
    ...
    if (!onceKey) return json(400, { error: "missing_once_key" });
    ...
    if (destinatari.length === 0) return json(200, { sent: 0, failed: 0, note: "no_recipients" });
    ...
    for (const [i, d] of destinatari.entries()) {
      if (i > 0) await pauza(PAUZA_ANUNT_MS);
      const body = fillVars(text, d.nume, d.email);
      const r = await trimiteCuReincercare(
        { to: d.email, subject, text: body },
        badge,
        linkDezabonarePagina(d.token_unsub),
        linkDezabonareApi(d.token_unsub)
      );
      if (r.ok) sent++;

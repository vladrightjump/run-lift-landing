...
import { anunt, rejoacaAnunt, type DepsAnunt } from "./anunt.ts";
...
const depsAnunt: DepsAnunt = {
  rpc,
  sendOne,
  logSends,
  loadConfig,
  loadBadge,
  fillVars: (text, nume, email) => fillVars(text, nume, email),
...
Deno.serve(async (req: Request) => {
  ...
  if (mode === "replay") {
    ...
    // Un anunț NU se reconstruiește din șablon — vezi `rejoacaAnunt`.
    if (!plan.ok && plan.mod === "anunt") {
      const r = await rejoacaAnunt(token, logId, depsAnunt);
  ...
  // Logica stă în `anunt.ts`, ca să fie testabilă din vitest.
  if (mode === "anunt") {
    const r = await anunt(payload, depsAnunt);
    return json(r.status, r.body);
  }

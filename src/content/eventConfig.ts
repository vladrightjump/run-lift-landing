...
/**
 * Textele secțiunii „Instagram". Clipurile NU stau aici — sunt fișiere proprii,
 * administrate din `/admin` → Clipuri, fiindcă nu țin de ediție.
 */
type ReelsConfig = {
  headline: string;
  body: string;
...
const parseReels = (raw: unknown): ReelsConfig => {
  if (!isRecord(raw)) return { ...DEFAULT_REELS };
